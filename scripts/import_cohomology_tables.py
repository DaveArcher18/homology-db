...
# The display strings are generated by the same code that generates them for the
# literature corpus, so a presentation is never authored beside the algebra it
# describes and the two corpora cannot drift into different notations.
from homology_db.cohomology_rings import MAX_PRESENTATION_TEX, presentation_strings

COEFFICIENTS = ("Z", "Q", "F2", "F3", "F5", "F7", "F11")
SCHEMA_VERSION = "homology-db.imported-models/1"
PRESENTATION_NOTE = (
    "Generators and relations were computed by the engine together with the "
    "multiplication table and imported with it. This repository re-derives that "
    "every stated relation holds in that table; that the generators generate and "
    "that the relations are complete are imported, not checked here."
)
...
CHECKED_IN = {
    "T^2": "torus:2", "Klein bottle": "klein_bottle",
    **{f"Sigma_{g}": f"orientable_surface:{g}" for g in range(2, 7)},
    **{f"N_{k}": f"nonorientable_surface:{k}" for k in range(3, 7)},
}

EXISTING = set(EXISTING_SPACE_ID) | set(CHECKED_IN)
...
def checked_in_entries(records: Path, manifest: Path) -> list[dict]:
    """Entries for the surfaces whose triangulations this repository ships.

    These are absent from `build_plan`, which describes only the models that are
    identified rather than redistributed.  Their rings are imported all the same,
    and are bound to the checked-in facet list by its hash, so the descriptors are
    read back out of the corpus manifest that ships those facets.
    """
    models = json.loads(manifest.read_text(encoding="utf-8"))["models"]
    by_space = {model["space_id"]: model for model in models}
    rows = {row["space"]: row for row in
            csv.DictReader((records / "MANIFEST.tsv").open(), delimiter="\t")}
    entries = []
    for name, space_id in sorted(CHECKED_IN.items(), key=lambda item: item[1]):
        model = by_space[space_id]
        row = rows[name]
        f_vector = [int(value) for value in row["f_vector"].split(",")]
        if model["facets_sha256"] != row["facets_sha256"]:
            raise ValueError(
                f"{space_id}: the shipped triangulation is {model['facets_sha256']} "
                f"but the producer computed on {row['facets_sha256']}")
        entries.append({
            "space_id": space_id, "source_space": name,
...
_FACTOR = re.compile(r"([A-Za-z_][A-Za-z0-9_]*)(?:\^(\d+))?\Z")
_SCALAR = re.compile(r"(\d+)(?://(\d+))?\Z")


def _split_terms(relation):
    """Signed terms of a producer relation string, in written order."""
    terms, sign, current = [], 1, ""
    for character in relation:
        if character in "+-" and current.strip():
            terms.append((sign, current))
            sign, current = (1 if character == "+" else -1), ""
        elif character in "+-":
...
def _monomial(term, degrees):
    """One term as (scalar, powers), with the Koszul sign of the reordering.

    The producer writes a product in a definite order; the atlas stores an
    unordered exponent map, which the validator evaluates in the order the
    generators are declared.  Reordering the factors is therefore not free --
    moving one odd class past another costs a sign.

    Measured over the corpus the producer already emits every monomial in
    declared-generator order, so the sign below never fires on today's data and
    the loop is a guard rather than a fix.  It is kept because the ordering is
    the producer's convention rather than anything this schema requires, and
    because the failure it prevents is silent: sorting the factors by name
    instead, which is what building the exponent map naively would do, reorders
    426 terms and falsifies 137 relations -- "x1_11" sorts before "x1_3".
    """
    scalar, factors = Fraction(1), []
    for piece in term.split("*"):
        piece = piece.strip()
        if not piece:
            continue
        number = _SCALAR.fullmatch(piece)
        if number:
            scalar *= Fraction(int(number[1]), int(number[2] or 1))
            continue
        match = _FACTOR.fullmatch(piece)
        if not match:
            raise ValueError(f"unreadable factor {piece!r} in relation term {term!r}")
        if match[1] not in degrees:
            raise ValueError(f"relation names {match[1]!r}, which is not a generator")
        factors.extend([match[1]] * int(match[2] or 1))

    # Insertion sort into declared-generator order, counting transpositions of
    # two odd-degree factors. Insertion sort rather than a key sort because the
    # sign is a property of the swaps performed, not of the final arrangement.
    order = {name: index for index, name in enumerate(degrees)}
    for i in range(1, len(factors)):
        j = i
        while j > 0 and order[factors[j - 1]] > order[factors[j]]:
            if degrees[factors[j - 1]] % 2 and degrees[factors[j]] % 2:
                scalar = -scalar
...
def _presentation(source, basis):
    """The producer's presentation, as generators and relations the atlas checks.

    Returns ``None`` when the producer found none: the ring is then recorded as a
    basis with its cup-product table, which is what the atlas already shows, and
    absence is never rendered as a claim that no presentation exists.
    """
    structured = source.get("ring_presentation_structured")
    if not structured:
        # A ring whose only class is the unit is the coefficient ring itself.
        # That needs no search to establish and the producer emits no structure
        # for it, so it is derived here rather than left unpresented.
        if [item["id"] for item in basis] == ["1"]:
            return [], []
        return None
    generators = [{"id": item["label"], "degree": item["degree"]}
                  for item in structured["generators"]]
    degrees = {item["id"]: item["degree"] for item in generators}
    relations = []
    for relation in structured["relations"]:
        terms = []
        for sign, text in _split_terms(relation):
            scalar, powers = _monomial(text, degrees)
            scalar *= sign
            terms.append({"coefficient": int(scalar) if scalar.denominator == 1 else str(scalar),
                          "powers": powers})
        if not terms:
            raise ValueError(f"empty relation {relation!r}")
        relations.append({"terms": terms})
    ...
    for entry in entries:
        ...
        for coefficient in COEFFICIENTS:
            ...
            if complete:
                ...
                presented = _presentation(source, basis)
                if presented is not None:
                    algebra["generators"], algebra["relations"] = presented
                    tex, plain = presentation_strings(algebra, coefficient)
                    record["presentation"] = {
                        "tex": tex if len(tex) <= MAX_PRESENTATION_TEX else "",
                        "plain": plain,
...
RINGS_SCHEMA = "homology-db.computed-rings-corpus/1"


def emit_rings(records: Path, manifest: Path) -> tuple[dict, dict]:
    """The ring and homology corpus, plus the spaces whose rings were withheld.

    The withheld map is the manifest's record of what the producer could not
    complete, and is returned rather than dropped so that an absence is always
    accompanied by its reason.
    """
    from homology_db.cohomology_rings import expected_groups

    # The model descriptors, not the raw plan: `build_records` needs the
    # dimension, which is read off the f-vector while the models are built.
    entries = build_models(records)["models"]
    entries += checked_in_entries(records, manifest)
    entries.sort(key=lambda entry: entry["space_id"])
    rings, homology, withheld = build_records(records, entries, expected_groups)
    return ({"schema_version": RINGS_SCHEMA, "records": rings, "homology": homology},
            withheld)
...
def main() -> int:
    ...
    parser.add_argument("--output", type=Path, required=True,
                        help="corpus/computed-rings-v1/ directory, or a file with --emit models")
    parser.add_argument("--emit", choices=("models", "rings", "all"), default="models",
                        help="which corpus files to write (default: models, as before)")
    arguments = parser.parse_args()

    # `--emit models` keeps writing a single named file, which is how this script
    # has always been called. The other modes write a corpus directory, because
    # rings.json and imported-models.json are only meaningful together.
    if arguments.emit == "models":
    ...
    if not directory.is_dir():
        raise SystemExit(f"--emit {arguments.emit} needs a corpus directory, not {directory}")
    if arguments.emit == "all":
        models = build_models(arguments.records)
        _write(directory / "imported-models.json", models)
        print(f"wrote {len(models['models'])} model descriptors")
    corpus, withheld = emit_rings(arguments.records, directory / "manifest.json")
    _write(directory / "rings.json", corpus)
    spaces = len({record["space_id"] for record in corpus["records"]})
    print(f"wrote {len(corpus['records'])} ring records over {spaces} spaces "
          f"and {len(corpus['homology'])} homology records")
    presented = sum(1 for record in corpus["records"] if "presentation" in record)
    shown = sum(1 for record in corpus["records"]
                if record.get("presentation", {}).get("tex"))
    print(f"  {presented} carry a presentation, {shown} of them with a TeX display")
    for space_id, reason in sorted(withheld.items()):
        print(f"  withheld rings for {space_id}: {reason}")
    return 0
