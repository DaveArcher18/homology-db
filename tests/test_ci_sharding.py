...
class CiShardingTest(unittest.TestCase):
    def test_four_shards_cover_exactly_the_discovered_suite(self) -> None:
        discovered = discover_tests()
        plan = build_plan(discovered, 4)
    ...
    def test_plan_is_deterministic_and_preserves_modules(self) -> None:
        discovered = discover_tests()
        first = build_plan(discovered, 4)
        second = build_plan(discovered, 4)
