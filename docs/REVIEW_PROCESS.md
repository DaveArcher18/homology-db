...
Pages gate accepts the stable-spectrum artifact only when that external record
says `accept` and every binding can be independently reconstructed from the
artifact and pinned source. On acceptance, those reviews and admissions are
...
artifact byte for byte. Development never manufactures the record. Until then
a local review artifact may exist, but `dist/atlas.html` remains the last
