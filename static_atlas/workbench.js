...
(() => {
  ...
  function scopeDescription(scope) {
    if(scope === "entire_rule")return "Entire family rule: all supported instances, coefficients, degrees and components";
    const n=scope?.parameters?.n || {}, d=scope?.degrees || {};
    return `n ${n.min}–${n.max ?? "all above"}; degrees ${d.min}–${d.max ?? "all above"}; ${(scope?.coefficients||[]).join(", ")}; ${(scope?.components||[]).join(", ")}`;
  }
  function reviewPanel(context,state,record,first,digest,refs) {
    const panel=e("details","wb-review-panel");panel.id="family-review";panel.append(e("summary","","Review this result"),e("p","wb-muted","Choose exactly what you checked. A review of one instance or coefficient does not approve the whole family. Submissions are public on GitHub and require maintainer validation; copying a packet does not submit or publish anything."));
    const form=e("form","review-scope-form");const modeLabel=e("label","wb-field","Review scope");const mode=e("select");mode.setAttribute("aria-label","Review scope");[["instance","This instance, selected degrees and coefficients"],["whole","Entire family rule — all instances and coefficients"]].forEach(([value,text])=>{const option=e("option","",text);option.value=value;mode.append(option);});modeLabel.append(mode);form.append(modeLabel);
    const narrow=e("div","review-narrow-scope");narrow.append(e("p","",`Instance: n = ${state.n}. Review scope covers ordinary unreduced homology and cohomology. For multiplication, both input degrees and the output degree must lie in the selected interval.`));
    const minLabel=e("label","wb-field","First reviewed degree");const min=e("input");min.inputMode="numeric";min.pattern="[0-9]+";min.required=true;min.value=state.start;minLabel.append(min);
    const maxLabel=e("label","wb-field","Last reviewed degree (blank = all above)");const max=e("input");max.inputMode="numeric";max.pattern="[0-9]*";max.value=String(BigInt(state.start)+8n);maxLabel.append(max);narrow.append(minLabel,maxLabel);
    function choices(title,values){const field=e("fieldset","review-choices");field.append(e("legend","",title));const inputs=values.map(value=>{const label=e("label");const input=e("input");input.type="checkbox";input.value=value;input.checked=true;input.setAttribute("aria-label",value);label.append(input," ");if(title==="Coefficients checked")label.append(context.renderTex(window.AtlasFamilies.coefficientTex(value),value,"math-inline"));else label.append(value);field.append(label);return input;});narrow.append(field);return inputs;}
    const coefficients=choices("Coefficients checked",["Z","Q","F2","F3","F5","F7","F11"]);const components=choices("Components checked",["homology","cohomology","multiplication"]);form.append(narrow);
    const summary=e("p","review-scope-summary");summary.setAttribute("role","status");const submit=link("Open public GitHub review form ↗","#");submit.target="_blank";submit.rel="noopener noreferrer";const copy=e("button","text-button","Copy review packet");copy.type="button";const error=e("p","review-scope-error");error.setAttribute("role","alert");form.append(summary,error,submit,copy);panel.append(form);
    let packet="", valid=true;
    function update(){narrow.hidden=mode.value==="whole";let scope="entire_rule";valid=true;if(mode.value!=="whole"){const cs=coefficients.filter(i=>i.checked).map(i=>i.value),ts=components.filter(i=>i.checked).map(i=>i.value);valid=/^\d+$/.test(min.value)&&/^\d*$/.test(max.value)&&cs.length>0&&ts.length>0;if(valid&&max.value)valid=BigInt(max.value)>=BigInt(min.value);if(valid)scope={parameters:{n:{min:state.n,max:state.n}},coefficients:cs,degrees:{min:BigInt(min.value).toString(),max:max.value?BigInt(max.value).toString():null},components:ts};}
      error.textContent=valid?"":"Choose at least one coefficient and component, and a valid nonnegative degree interval.";copy.disabled=!valid;submit.setAttribute("aria-disabled",String(!valid));if(!valid){submit.removeAttribute("href");summary.textContent="";return;}
      summary.textContent=scopeDescription(scope);const sourceText=(Array.isArray(refs)?refs:Object.values(refs)).map(ref=>`${ref.title}: ${ref.url} — ${ref.locator}`).join("\n");const permalink="https://davearcher18.github.io/homology-db/"+hashFor(state);
      packet=`Homology Atlas review packet\nFamily: ${state.family}\nRule: ${first.rule_id}\nVersion: ${first.rule_version}\nHash: ${digest}\nScope: ${JSON.stringify(scope)}\nReadable scope: ${scopeDescription(scope)}\nResult convention: ordinary unreduced homology and cohomology. Multiplication requires both input degrees and output degree inside the selected interval. Reduced-only display feedback should be described in the rationale; it is not acceptance of the ordinary results.\nView: ${permalink}\nSources:\n${sourceText}\n\nReviewer name:\nVerdict (accept / needs-evidence / reject):\nRationale and exact corrections:\n\nNot a published review until a maintainer validates the named human submission.`;
      submit.href="https://github.com/DaveArcher18/homology-db/issues/new?"+new URLSearchParams({template:"family-review.yml",title:`Family review: ${first.rule_id} v${first.rule_version}`,"rule-id":first.rule_id,"rule-version":first.rule_version,"rule-hash":digest,scope:typeof scope==="string"?scope:JSON.stringify(scope),references:sourceText.slice(0,600)+"\nView: "+permalink});}
    form.addEventListener("input",update);form.addEventListener("change",update);form.addEventListener("submit",event=>event.preventDefault());copy.addEventListener("click",()=>{update();if(valid)context.copyText(packet,copy);});update();return panel;
  }
  ...
  function create(context, input) {
    ...
    if((record.scoped_human_reviews||[]).length)titleRow.append(e("span","wb-review-state",record.scoped_review_state==="scoped_human_concern"?"Concern recorded in a narrower scope — see review details":"Narrow-scope reviews recorded — see review details"));
    intro.append(titleRow);view.append(intro);
    const teaching=atlas.teaching?.entries?.find(entry=>entry.space_id===`${state.family}:${state.n}`);
    if(teaching){const description=e("p","wb-teaching-intro");mathText(description,teaching.introduction,math);intro.append(description);const notes=e("details","space-teaching-note");notes.append(e("summary","","What to notice"));const body=e("p");mathText(body,teaching.teaching_point,math);notes.append(body);(teaching.sources||[]).forEach(ref=>{const p=e("p","wb-muted");p.append(link(ref.title,ref.url)," · "+ref.locator);notes.append(p);});intro.append(notes);}
    ...
    utilities.append(link("Textbook map & comparisons →","#textbook"));
    ...
    const review=reviewPanel(context,state,record,first,digest,refs);
    view.append(review);
    const scopedReviews=record.scoped_human_reviews || [];
    if(scopedReviews.length){const scoped=e("section","wb-scoped-reviews");scoped.append(e("h3","","Reviews of narrower scopes"),e("p","wb-muted","These verdicts apply only to the stated scope, not the entire family rule."));
      scopedReviews.forEach(item=>{const card=e("article","wb-scoped-review");card.append(e("p","",`${item.reviewer}: ${item.verdict}`),e("p","",scopeDescription(item.scope)),e("p","",item.rationale));if(/^https:\/\//.test(item.source_url))card.append(link("Review evidence",item.source_url));scoped.append(card);});source.append(scoped);}
    ...
    download.setAttribute("aria-label","Download family JSON");download.textContent="JSON ↓";
    const reviewTrigger=e("button","text-button","Review this result");reviewTrigger.type="button";reviewTrigger.addEventListener("click",()=>{review.open=true;review.scrollIntoView({block:"start"});review.querySelector("summary").focus({preventScroll:true});});
    const quickActions=e("div","wb-reading-actions");quickActions.append(reviewTrigger,download);utilities.insertAdjacentElement("afterend",quickActions);
